...
type AIConfigService struct {
	...
	cfg      *config.Config
	defaultP ai.Provider // env 系统默认 provider(ResolveProvider 的回退目标由调用方处理)
...
func NewAIConfigService(repo repository.AIConfigRepository, cfg *config.Config, defaultP ai.Provider) *AIConfigService {
...
func (s *AIConfigService) ResolveProvider(ctx context.Context, username string) ai.Provider {
	if s == nil || username == "" {
		return nil
	}
	...
	key, err := util.DecryptSecret(s.cfg.AIConfigSecretKey(), cfg.APIKeyEnc)
	if err != nil || key == "" {
...
func (s *AIConfigService) GetView(ctx context.Context, username string) (*model.AIConfigView, error) {
	...
	if cfg.APIKeyEnc != "" {
		if key, err := util.DecryptSecret(s.cfg.AIConfigSecretKey(), cfg.APIKeyEnc); err == nil {
			view.APIKeyMask = util.MaskSecret(key)
		}
	}
	view.Mode = "openai"
	return view, nil
...
func (s *AIConfigService) Save(ctx context.Context, username, baseURL, apiKey, chatModel, embedModel string) error {
	if baseURL != "" {
		if err := util.ValidatePublicURL(baseURL, s.cfg.AllowPrivateAIURL); err != nil {
			return err
		}
	}
	...
	if apiKey != "" {
		enc, err := util.EncryptSecret(s.cfg.AIConfigSecretKey(), apiKey)
		if err != nil {
			return err
		}
		cfg.APIKeyEnc = enc

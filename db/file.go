...
func OnFileUploadFinished(filehash string, filename string, filesize int64, fileaddr string, uploadAt time.Time) bool {
	stmt, err := mydb.DBConn().Prepare(
		"INSERT IGNORE INTO tbl_file(`file_sha1`,`file_name`,`file_size`,`file_addr`,`create_at`,status) VALUES(?,?,?,?,?,1)")
	if err != nil {
...
func GetFileMeta(filehash string) (fileMeta *TableFile, err error) {
	stmt, err := mydb.DBConn().Prepare(
		"SELECT file_sha1, file_name, file_size, file_addr, create_at FROM tbl_file WHERE file_sha1=? AND status=1 LIMIT 1")
	if err != nil {
	...
	tfile := TableFile{}
	err = stmt.QueryRow(filehash).Scan(&tfile.FileSha1, &tfile.FileName, &tfile.FileSize, &tfile.FileAddr, &tfile.CreateAt)
	if err != nil {
		return nil, err
	}
	return &tfile, nil
...
func GetAllFileMeta() ([]TableFile, error) {
	rows, err := mydb.DBConn().Query(
		"SELECT file_sha1, file_name, file_size, file_addr, create_at FROM tbl_file WHERE status=1 ORDER BY create_at DESC")
	if err != nil {
	...
	for rows.Next() {
		var tfile TableFile
		if err := rows.Scan(&tfile.FileSha1, &tfile.FileName, &tfile.FileSize, &tfile.FileAddr, &tfile.CreateAt); err != nil {

...
function pick(f) {
  if (!f) return
  if (f.size > MAX_FILE) { toast('文件过大（最大 100MB）', 'err'); return }
  selected.value = f
}
function onDrop(e) {
...
function clear() { selected.value = null; cancelFlag.value = false }
function cancel() { cancelFlag.value = true; toast('已取消', 'warn') }
...
async function start() {
  ...
  try {
    ...
    // 秒传检测:meta 命中即已存在
    try {
      await api('/file/meta?filehash=' + encodeURIComponent(hash))
      toast('秒传成功', 'ok')
      emit('changed')
      clear()
      return
    ...
    if (file.size >= CHUNK_MIN) {
      // 分片上传 + 断点续传
      let uploaded = []
      try { uploaded = (await api('/file/upload/status?filehash=' + encodeURIComponent(hash)) || []).map(String) } catch { /* 无历史 */ }
      ...
      fd.append('filehash', hash)
      fd.append('file', file)
      await apiUpload('/file/upload', fd, (p) => { pct.value = p })
    }
    toast('上传完成', 'ok')
    emit('changed')
  } catch (e) {
...
<template>
  <div>
    <div
      ...
      @drop.prevent="onDrop"
    >
      <input ref="fileInput" type="file" hidden @change="e => pick(e.target.files[0])">
